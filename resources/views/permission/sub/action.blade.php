<div class="btn-group float-right">
    <a href="#" class="btn btn-info" title="{{ trans('alpaca::alpaca.edit') }}"
       v-b-modal.modalpermissionedit{{ $permission->id }}>
        <i class="fa fa-pencil-square-o" aria-hidden="true"></i>
    </a>

    <a href="#" class="btn btn-danger" title="{{ trans('alpaca::alpaca.delete') }}"
       v-b-modal.modalpermissiondelete{{ $permission->id }}>
...
{{--Delete Component--}}
<b-modal id="modalpermissiondelete{{ $permission->id }}" title="{{ trans('alpaca::alpaca.sure_delete') }}" hide-footer>

    <form method="POST" action="/backend/permission/{{ $permission->id }}" accept-charset="UTF-8" aria-label="Action">
